...
def integrate_rk4(plant, t_start: float, t_stop: float, dt: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    differentiable, so RK4 is not strictly appropriate for stochastic differential equations (SDEs). 
    However, we can still use RK4 as a numerical integration method for SDEs by treating the 
    noise as constant across each `dt_int` (additive drift, rather than diffusion). This has the 
    effect of scaling the noise by a factor of `sqrt(dt_int)`, and results are therefore not 
    comparable between RK4 and the other methods, which solve the true SDE.
    """
...
def integrate_euler_maruyama(plant, t_start: float, t_stop: float, dt: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    NOTE: if needed, we can compute G using the Cholesky decomposition of Q, 
    since Q is positive semidefinite. However, drawing samples from N(0, Q) is more efficient
    (and mathemtically equivalent) than drawing samples from N(0, I) and multiplying by G.
    """
