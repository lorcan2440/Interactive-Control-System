...
class PlantModel:
    ...
    def system_dynamics(self, t: float, x: np.ndarray):
        ...
        w1 = np.random.normal(0, self.simulator.w1_stddev)  # process noise
        w2 = np.random.normal(0, self.simulator.w2_stddev)  # measurement noise
        
        # noisy measurement
        y_noisy = x[1] + w2
        
        # use the appropriate controller - it only sees the noisy measurement, not the actual state
        match self.simulator.controller_type:
            case ControllerType.MANUAL:
                u = self.simulator.manual_controller.calc_u(t, y_noisy)
            case ControllerType.PID:
                u = self.simulator.pid_controller.calc_u(t, y_noisy)
            case ControllerType.H2:
                u = self.simulator.h2_controller.calc_u(t, y_noisy)
            case _:
